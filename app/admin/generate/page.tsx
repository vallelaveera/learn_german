...
type PageTab = "generate" | "coverage" | "taxonomy";

const PAGE_TABS = [
  { id: "generate" as const, label: "Generieren" },
  { id: "coverage" as const, label: "Abdeckung" },
  { id: "taxonomy" as const, label: "Taxonomie" },
];
...
function AdminGeneratePageInner() {
  ...
  return (
    <>
      <AdminSubTabs tabs={PAGE_TABS} active={pageTab} onChange={id => setPageTab(id as PageTab)} />

      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {pageTab === "coverage" && coverage && (
          <>
            <AdminCard>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: "var(--text)", marginBottom: 4 }}>Corpus-Matrix</div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{coverage.totals.words} W · {coverage.totals.sentences} S gesamt</div>
                </div>
              </div>
            ...
            {coverage.gaps.length > 0 && (
              <AdminCard style={{ background: "#FFFBEB", borderColor: "#FDE68A" }}>
                <div style={{ fontSize: 11, color: "#92400E", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>
                  Lücken — Klicken zum Vorausfüllen
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {(showAllCoverage ? coverage.gaps : coverage.gaps.slice(0, 8)).map(gap => (
                    <button
                      key={gap.category}
                      type="button"
                      onClick={() => { applyGap(gap); setPageTab("generate"); }}
                      disabled={running}
                      style={{
                        textAlign: "left",
                        background: "#FEF3C7",
                    ...
                    >
                      ...
                      <div style={{ fontSize: 11, color: "#B45309", fontFamily: "var(--font-mono)" }}>
                        {gap.words}/{coverage.targets.words} W · {gap.sentences}/{coverage.targets.sentences} S
                      </div>
                    </button>
                  ))}
                </div>
                {coverage.gaps.length > 8 && (
                  <button
                    ...
                    style={{ fontSize: 11, color: "#7F77DD", background: "none", border: "none", cursor: "pointer", padding: "8px 0 0", fontFamily: "var(--font-mono)" }}
                  >
                    {showAllCoverage ? "Weniger" : `Alle ${coverage.gaps.length} Lücken`}
                  </button>
                )}
              </AdminCard>
            )}
        ...
        {pageTab === "taxonomy" && (
          <AdminCard>
            <div style={{ fontSize: 14, fontWeight: 500, color: "var(--text)", marginBottom: 12 }}>Kategorien & Themen</div>
            <form onSubmit={handleAddCategory} style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
            ...
            <form onSubmit={handleAddTopic} style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
            ...
            {taxonomyError && (
              <p style={{ fontSize: 12, color: "#DC2626", margin: "0 0 12px" }}>{taxonomyError}</p>
            )}
