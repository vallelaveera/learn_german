...
import {
  pickRandomQuestion,
  type MayaQuestion,
} from "@/lib/content/maya-questions";
...
function truncateLine(text: string, max = 22): string {
...
export function HomeIllustration({
  ...
  const [question, setQuestion] = useState<MayaQuestion | null>(null);
  const [vocabPair, setVocabPair] = useState({
    de: "der Bahnhof",
    en: "train station",
  });
  ...
  useEffect(() => {
    ...
    if (recent.length > 0 && Math.random() < 0.4) {
      ...
      q = pickRandomQuestion(userLevel || "B1", recent);
    }

    setQuestion(q);
  }, [userLevel, recentMistakes]);

  useEffect(() => {
    const pair =
      VOCAB_PAIRS[Math.floor(Math.random() * VOCAB_PAIRS.length)];
    setVocabPair(pair);
  }, []);
  ...
  const bubbleWidth = Math.max(
    100,
    Math.max(line1.length, line2.length) * 5.5 + 24,
  );
  const bubbleHeight = twoLines ? 44 : 36;
  ...
  const dePillX = 18;
  const enPillX = 320 - enPillWidth - 14;
  ...
  `}</style>
      ...
      {/* Maya speech bubble — dynamic question */}
      <g className="bub1">
        <rect
          x={bubbleX}
          y="55"
        ...
        />
        <path
          d={`M${bubbleX} 72 L${bubbleX - 8} 76 L${bubbleX + 4} 76Z`}
          fill="white"
          stroke="#CECBF6"
          strokeWidth="1"
        />
        <path
          d={`M${bubbleX - 7} 76 L${bubbleX + 4} 76`}
          stroke="white"
          strokeWidth="2"
        />
        <text
